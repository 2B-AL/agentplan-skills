"""Resolve the standalone al-credential-sync runtime from its pinned official source."""
...
RUNTIME_ROOT = Path.home() / ".openclaw" / "ark-cua" / "dependencies" / "al-credential-sync"
...
def discover(config):
    candidates = []
    explicit = (
        os.environ.get("AP_CUA_CREDENTIAL_SKILL_DIR")
        or os.environ.get("CUA_CREDENTIAL_SKILL_DIR")
        or config.get("credential_skill_dir")
        or config.get("dir")
    )
    if explicit:
        candidates.append(Path(explicit).expanduser())
    # Normal AgentPlan use is tied to the managed, versioned cache. An explicit
    # path is a developer/operator override; never silently run another agent's
    # globally installed copy because it may not match the published pin.
    try:
        candidates.append(RUNTIME_ROOT / _source_config(config)["commit"])
    except SkillError:
        pass
...
def _source_config(config):
    repository = config.get("credential_skill_repository") or config.get("repository") or OFFICIAL_REPOSITORY
    ...
    commit = str(
        os.environ.get("AP_CUA_CREDENTIAL_SKILL_COMMIT")
...
def _download_github_archive(url):
    ...
    request = urllib.request.Request(url, headers={"User-Agent": "ark-cua/credential-dependency-v2"})
    try:
...
def _safe_extract(raw, destination):
    ...
    try:
        with zipfile.ZipFile(archive_path) as archive:
            ...
            if sum(info.file_size for info in infos) > MAX_EXPANDED_ARCHIVE:
